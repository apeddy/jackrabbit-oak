...
public class AwsArchiveManager implements SegmentArchiveManager {
    ...
    public SegmentArchiveReader forceOpen(String archiveName) throws IOException {
        return new AwsSegmentArchiveReader(awsContext.withDirectory(archiveName), archiveName, ioMonitor);
    }
    ...
    public boolean delete(String archiveName) {
        try {
            awsContext.withDirectory(archiveName).deleteAllObjects();
            return true;
        } catch (IOException e) {
    ...
    public boolean renameTo(String from, String to) {
        try {
            awsContext.withDirectory(from).listObjects("").forEach(obj -> {
                try {
                    awsContext.withDirectory(to).copyObject(obj.getKey());
                    awsContext.s3.deleteObject(new DeleteObjectRequest(awsContext.bucketName, obj.getKey()));
                } catch (IOException | AmazonServiceException e) {
                    log.error("Can't rename segment {}", obj.getKey(), e);
                }
            });
            return true;
        } catch (IOException e) {
    ...
    public boolean exists(String archiveName) {
        try {
            return !awsContext.withDirectory(archiveName).listObjects("").isEmpty();
        } catch (IOException e) {
    ...
    public void recoverEntries(String archiveName, LinkedHashMap<UUID, byte[]> entries) throws IOException {
        ...
        for (S3ObjectSummary b : awsContext.withDirectory(archiveName).listObjects("")) {
            ...
            try (S3Object object = awsContext.s3.getObject(awsContext.bucketName, b.getKey())) {
                byte[] data = IOUtils.readBytes(object.getObjectContent());
                if (data.length > 0) {
