...
public class AwsArchiveManagerTest {
    ...
    public void setup() throws IOException {
        bucketName = "testbucket-" + new Date().getTime();
        s3 = s3Mock.createClient();
        s3.createBucket(bucketName);

        String tableName = "testtable-" + new Date().getTime();
        lockTableName = "locktable-" + new Date().getTime();
        AmazonDynamoDB ddb = DynamoDBMock.createClient(tableName, lockTableName);

        awsContext = AwsContext.create(s3, bucketName, rootDirectory, ddb, tableName, lockTableName);
    }
